...
def plot_metrics(history, model_name):
    ...
    plt.title(f'{model_name} GAF Score')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(f"results/GID_Metrics_{model_name}.png")

# --- Read the NEW CSV files ---
try:
    unet_history = pd.read_csv('results/unet_gpu_gid_GID_model.csv').to_dict('records')
    plot_metrics(unet_history, 'U-Net')
except FileNotFoundError:
    print("U-Net CSV not found. Skipping.")

try:
    swin_history = pd.read_csv('results/swin_gpu_gid_GID_model.csv').to_dict('records')
    plot_metrics(swin_history, 'Swin-UNet')
except FileNotFoundError:
    print("Swin-UNet CSV not found. Skipping.")